...
func BenchmarkLoop(m dsl.Matcher) {
	...
	// Pattern 3: for range b.N (no variable)
	// Report only, no Suggest: the $*body group does not round-trip through
	// golangci-lint's gocritic --fix (it writes literal "$*body" and corrupts the
	// loop). Verified with golangci-lint 2.13.2.
	m.Match(
		`for range $b.N { $*body }`,
	).
		Where(m["b"].Type.Is("*testing.B")).
